...
/**
 * 2. Compile less to css. This must happen before we copy to tmp file in case references to node_modes was made
 */
gulp.task('less', function () {
  const dirs = [`${srcFolder}/**/*.less`, `!${srcFolder}/node_modules`, `!${srcFolder}/node_modules/**`];
  return gulp.src(dirs)
      .pipe(gulpLess({paths: dirs}))
      .pipe(gulp.dest((file) => file.base));
});

/**
 * 3. Clone the /src folder into /.tmp. If an npm link inside /src has been made,
 *    then it's likely that a node_modules folder exists. Ignore this folder
 *    when copying to /.tmp.
 */
gulp.task('copy:source', function () {
  return gulp.src([`${srcFolder}/**/*`, `!${srcFolder}/node_modules`, `!${srcFolder}/node_modules/**`])
